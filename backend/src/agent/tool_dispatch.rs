...
// Reserved for enforcing the sub-agent nesting depth limit. Its value (3) is the
// source of truth behind the "Maximum nesting depth: 3" note in the spawn_subagent
// tool schema. Nesting is currently disabled (sub-agents may not spawn their own
// sub-agents — see the Q6 guard in `execute_spawn_subagent`), so this constant is
// not yet enforced in code. Kept to avoid magic-number drift and to back the limit
// once nested sub-agents are reintroduced.
#[allow(dead_code)]
