...
除了交互式聊天，Coder 还支持基于 cron 的**定时自动化任务**、面向长任务的**上下文感知会话交接**、可定制的**技能系统**，以及通过任意 OpenAI 兼容 API 支持的**多模型提供商**。
...
| 能力 | 描述 |
|---|---|
| **定时任务** | 基于 cron 的 AI Agent 自动化，可配置模型、Agent 模式（Agent/Ask）、思考开关、邮件通知和运行历史记录。 |
...
│   │   ├── scheduled_jobs/            # Cron 调度器、Agent 循环运行器、运行追踪
│   │   ├── db/                        # SQLite 持久化层
...
创建基于 cron 的 AI 任务，自主运行并支持：

- 可配置的 cron 表达式
- 按任务选择模型和 Agent 模式（Agent/Ask）
- 可选的思考/推理开关
- 完成或失败时的邮件通知
- 最多记录 50 次运行历史
