...
describe("waitForAgentTaskCompletion", () => {
  ...
  it("returns a buffered terminal status immediately", async () => {
    await expect(
      waitForAgentTaskCompletion("task-1", "failed")
  ...
  it("falls back to agent status when the event was already emitted", async () => {
    vi.mocked(getAgentStatus).mockResolvedValue({
      taskId: "task-3",
      status: "completed",
    });

    await expect(waitForAgentTaskCompletion("task-3")).resolves.toBe("completed");
  });
});
