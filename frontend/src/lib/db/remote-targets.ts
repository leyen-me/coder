...
export async function listRemoteTargets(): Promise<RemoteTargetConfig[]> {
  ...
  const targets = await db.getAll<RemoteTargetConfig>(REMOTE_TARGETS_STORE);
...
export async function getRemoteTarget(
  ...
  const db = await getDb();
  return (await db.get<RemoteTargetConfig>(REMOTE_TARGETS_STORE, alias)) ?? null;
}
...
export async function deleteRemoteTarget(alias: string): Promise<boolean> {
  const db = await getDb();
  const existing = await db.get<RemoteTargetConfig>(REMOTE_TARGETS_STORE, alias);
  if (!existing) {
    return false;
  }
